                ...
                children: [
                  ...
                  Hero(
                    ...
                    child: Material(
                      type: MaterialType.transparency,
                      child: Text(
                        '${state.registros?.totalRegistros ?? 'error'}',
                        style:
                  ...
                  Hero(
                    ...
                    child: Material(
                      type: MaterialType.transparency,
                      child: Text(
                        '${state.registros?.totalRegistrosVencidos ?? 'error'}',
                        style:
                            Theme.of(context).textTheme.titleMedium!.copyWith(
                                  color: primary,
                                  fontSize: 15,
      ...
      floatingActionButton: BlocSelector<MainBloc, MainState, Registros?>(
        ...
        builder: (context, state) {
          ...
          return FloatingActionButton(
            onPressed: () {
              descarga.ahora(datos: state.registrosList, nombre: 'Registros');
            },
                  ...
                  child: BlocBuilder<MainBloc, MainState>(
                    builder: (context, state) {
                      List<RegistrosSingle> registros =
                          state.registros?.registrosListSearch ?? [];
                      int endList = (state.registros?.viewRegistros ?? 0) >
                              registros.length
                          ? registros.length
                          : state.registros?.viewRegistros ?? 0;
          ...
          children: [
            Expanded(
              ...
              child: DropdownButtonFormField(
                items: state.registros?.registrosList
                        .map((e) => e.solicitante)
                        .toSet()
                        .toList()
                        .map(
                ...
                onChanged: (value) {
                  context.read<MainBloc>().add(
                      Busqueda(value: value.toString(), table: "Registros"));
                },
                isExpanded: true,
