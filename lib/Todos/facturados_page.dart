...
class FacturadosPage extends StatefulWidget {
  const FacturadosPage({super.key});
  @override
                ...
                children: [
                  ...
                  Hero(
                    ...
                    child: Material(
                      type: MaterialType.transparency,
                      child: Text(
                        '${state.registros?.totalFactura ?? 'error'}',
                        style:
                  ...
                  Hero(
                    ...
                    child: Material(
                      type: MaterialType.transparency,
                      child: Text(
                        '${state.registros?.totalFacturaVencidos ?? 'error'}',
                        style:
                            Theme.of(context).textTheme.titleMedium!.copyWith(
                                  color: primary,
                                  fontSize: 15,
      ...
      persistentFooterButtons: [
        Row(
          ...
          children: [
            ...
            Text(
              version.status('Cerrados', '$runtimeType'),
              style: Theme.of(context).textTheme.labelSmall,
            )
          ],
        ),
      ...
      floatingActionButton: BlocSelector<MainBloc, MainState, Registros?>(
        ...
        builder: (context, state) {
          ...
          return FloatingActionButton(
            onPressed: () {
              descarga.ahora(datos: state.facturaList, nombre: 'Facturas');
            },
                  ...
                  child: BlocBuilder<MainBloc, MainState>(
                    builder: (context, state) {
                      List<RegistrosSingle> registros =
                          state.registros?.facturaListSearch ?? [];
                      int endList = (state.registros?.viewFacturas ?? 0) >
                              registros.length
                          ? registros.length
                          : state.registros?.viewFacturas ?? 0;
          ...
          children: [
            Expanded(
              ...
              child: DropdownButtonFormField(
                items: state.registros?.facturaList 
                        .map((e) => e.solicitante)
                        .toSet()
                        .toList()
                        .map(
                ...
                onChanged: (value) {
                  context.read<MainBloc>().add(
                      Busqueda(value: value.toString(), table: "Facturas"));
                },
                isExpanded: true,
