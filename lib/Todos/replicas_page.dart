                ...
                children: [
                  ...
                  Hero(
                    ...
                    child: Material(
                      type: MaterialType.transparency,
                      child: Text(
                        '${state.registros?.totalReplica ?? 'error'}',
                        style:
                  ...
                  Hero(
                    ...
                    child: Material(
                      type: MaterialType.transparency,
                      child: Text(
                        '${state.registros?.totalReplicaVencidos ?? 'error'}',
                        style:
                            Theme.of(context).textTheme.titleMedium!.copyWith(
                                  color: primary,
                                  fontSize: 15,
      ...
      persistentFooterButtons: [
        Row(
          ...
          children: [
            ...
            Text(
              version.status('Réplicas', '$runtimeType'),
              style: Theme.of(context).textTheme.labelSmall,
            )
          ],
        ),
      ...
      floatingActionButton: BlocSelector<MainBloc, MainState, Registros?>(
        ...
        builder: (context, state) {
          ...
          return FloatingActionButton(
            onPressed: () {
              descarga.ahora(datos: state.replicaList, nombre: 'Replicas');
            },
                  ...
                  child: BlocBuilder<MainBloc, MainState>(
                    builder: (context, state) {
                      List<RegistrosSingle> registros =
                          state.registros?.replicaListSearch ?? [];
                      int endList = (state.registros?.viewReplicas ?? 0) >
                              registros.length
                          ? registros.length
                          : state.registros?.viewReplicas ?? 0;
          ...
          children: [
            Expanded(
              ...
              child: DropdownButtonFormField(
                items: state.registros?.replicaList 
                        .map((e) => e.solicitante)
                        .toSet()
                        .toList()
                        .map(
                ...
                onChanged: (value) {
                  context.read<MainBloc>().add(
                      Busqueda(value: value.toString(), table: "Replicas"));
                },
                isExpanded: true,
